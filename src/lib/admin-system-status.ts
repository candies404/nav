...
export type StatusLevel = 'available' | 'degraded' | 'unconfigured' | 'error'
...
export async function getAdminSystemStatus(
  ...
  const cached = globalSystemStatusCache.__navsphereAdminSystemStatusCache
  ...
  const result: AdminSystemStatus = {
    ...
    services: [redis, blob],
    capabilities: buildCapabilities(redis),
  }

  globalSystemStatusCache.__navsphereAdminSystemStatusCache = {
    value: result,
    expiresAt: Date.now() + SYSTEM_STATUS_CACHE_TTL_MS,
  }

  return result
}

async function checkRedisStatus(): Promise<SystemStatusItem> {
...
async function checkBlobStatus(): Promise<SystemStatusItem> {
  if (!BLOB_TOKEN) {
    return {
      id: 'blob',
      title: 'Vercel Blob',
...
function buildCapabilities(redis: SystemStatusItem): SystemStatusItem[] {
