...
export function NavigationClient({ initialItems }: { initialItems: NavigationItem[] }) {
  ...
  const refreshItems = useCallback(
    () => mutate(
      fetcher('/api/navigation?view=summary&fresh=1'),
      { revalidate: false }
    ),
    [mutate]
  )
  ...
  const handleAdd = async (values: {
    ...
    try {
      ...
      const result = await response.json() as { item: NavigationItem }

      setIsDialogOpen(false)
      await mutate(
        currentItems => [...(currentItems || []), result.item],
        { revalidate: false }
      )
      toast({
        title: "成功",
